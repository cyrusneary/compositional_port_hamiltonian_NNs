...
def get_dataset(experiment_name, save_path, **kwargs):
    ...
    print("Building dataset...".format(save_path))
...
def main():
    ...
    path_path = os.path.join(
        ...
        'simulated_data',
        datetime.now().strftime('pendulum_pixels' + '_%Y-%m-%d-%H-%M-%S.pkl')
    )
    get_dataset("pendulum", path_path)
