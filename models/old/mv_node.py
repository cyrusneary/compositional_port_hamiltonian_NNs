...
from models.old.neural_ode import NODE
    ...
    def _build_neural_ode(self):
        ...
        def forward(params, x):
            ...
            def f_approximator(x : jnp.ndarray, t=0):
                ...
                # dh_dq, dh_dp = jnp.split(dh, 2, -1)
                # return jnp.concatenate((dh_dp, -dh_dq), axis=-1)

                # q, p = jnp.split(x, 2, -1)
                # dh_dq = jax.vmap(jax.grad(H, argnums=0), in_axes=(0,0,None))(q, p, params)
                # dh_dp = jax.vmap(jax.grad(H, argnums=1), in_axes=(0, 0, None))(q, p, params)
